...
# VerdictReason.detail is a narrative field: single 20000-char safety
# ceiling (NARRATIVE_MAX), not a content limit. See DECISIONS 2026-05-19.
CEILING = 20000


def test_verdict_reason_detail_at_ceiling_accepted() -> None:
    r = VerdictReason(
        code="citation_mismatch",
        detail="x" * CEILING,
        audit_entries_examined=[1],
    )
    assert len(r.detail) == CEILING


def test_verdict_reason_detail_above_ceiling_rejected() -> None:
    with pytest.raises(ValidationError):
        VerdictReason(
            code="citation_mismatch",
            detail="x" * (CEILING + 1),
...
if __name__ == "__main__":
    ...
    test_verdict_reason_detail_at_ceiling_accepted()
    test_verdict_reason_detail_above_ceiling_rejected()
